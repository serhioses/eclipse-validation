...
validator = {
  ...
  checked(field, val, options) {
    ...
    field.each(function () {
      if ($(this).prop('checked') === val && $(this).val() !== 'unselectable') {
        counter += 1;
      }
    });
...
function initElements () {
  this._defaults.names.forEach((item) => {
    ...
    this.parents[item] = this.elements[item].closest('.ev-field-parent');
  });
}
  ...
  if (group.countValid >= group.requiredFields && !checkEmpty) {
    ...
    groupElements.forEach((el) => {
      if (this.elements[el].data('ev-possibly-valid')) {
        ...
        this.parents[el].addClass('ev-valid-item');
        defaults.onSetValid();
      }
    });
  } else {
    ...
    try {
      groupElements.forEach((el) => {
        if (this.elements[el].data('ev-possibly-error')) {
          ...
          this.parents[el].addClass('ev-invalid-item');
          ...
          if (!this.firstInvalidField) {
            ...
            this.firstInvalidField.addClass('ev-invalid-item--first').closest('.ev-field-parent').addClass('ev-invalid-item--first');
  ...
  try {
    this._defaults.names.forEach((el) => {
      ...
      if (typeof rules.group !== 'undefined') {
        ...
        this.elements[el].removeClass('ev-valid-item');
        this.parents[el].removeClass('ev-valid-item');
        ...
        if (!result.value && shouldCheck) {
          ...
          if (!this.firstInvalidField) {
            this.firstInvalidField = this.elements[el];
            this.firstInvalidField.addClass('ev-invalid-item--first').closest('.ev-notice-parent').addClass('ev-invalid-item--first');
...
export default class Validation {
  ...
  validate(e) {
    ...
    Object.keys(this.elements).forEach((el) => {
      this.elements[el].removeClass('ev-invalid-item ev-valid-item ev-valid-item--first ev-invalid-item--first').data('ev-possibly-error', '').data('ev-possibly-valid', '');
      this.parents[el].removeClass('ev-invalid-item ev-valid-item ev-valid-item--first ev-invalid-item--first');
    });
